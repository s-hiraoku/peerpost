...
`doctor` prints actionable fixes when something is wrong, including CLI/daemon version mismatches, stale autostart files, and local permission issues. With `--team`, it also reports missing agent registrations, pending deliveries, and unregistered recipient ids. For scripts:
...
Hook, monitor, and plain message output strips ANSI and other terminal control characters. Message bodies preserve ordinary newlines and tabs with indentation; metadata fields such as agent and team ids are rendered as single-line fields.
