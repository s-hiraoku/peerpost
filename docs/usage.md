...
It reports home directory permissions; `doctor --fix` restores `PEERPOST_HOME` to private mode.
It reports backup directory permissions; `doctor --fix` restores `<PEERPOST_HOME>/backups` to private mode.
It also warns if `PEERPOST_SOCKET` is too long for a Unix domain socket; use a short `/tmp/peerpost-...sock` path. Client commands also report `socket path too long` for this configuration error.
...
Use `--backup-first` with `--apply` to create and verify a backup before deleting. If the backup verification fails, `prune` exits nonzero and does not delete messages. Use `--backup-output <path>` with `--backup-first` to choose the backup file.
